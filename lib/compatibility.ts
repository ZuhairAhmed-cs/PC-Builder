...
export function checkCompatibility(
  ...
  // Rule 1: CPU socket matches Motherboard socket
  ...
  // Rule 2: Motherboard form factor fits Case
  if (build.motherboard && build.case) {
    ...
    if (
      moboFormFactor &&
      !caseSupportedFormFactors.includes(moboFormFactor)
    ) {
      errors.push({
        type: "error",
        title: "Form Factor Mismatch",
        description: `${build.motherboard.title} is ${moboFormFactor}, but ${build.case.title} only supports ${caseSupportedFormFactors.join(", ")}.`,
        affectedComponents: ["motherboard", "case"],
      });
    }
  }

  // Rule 3: PSU wattage is sufficient
  if (build.psu) {
    ...
    if (totalPower > psuWattage) {
      ...
      warnings.push({
        ...
        description: `Your build uses ${totalPower}W with a ${psuWattage}W PSU (${Math.round(headroomPercent)}% headroom). Consider a higher wattage PSU for better efficiency and future upgrades.`,
        affectedComponents: ["psu"],
      });
    }
  }

  // Rule 4: Check for high-power GPU without adequate PSU
  ...
  // Rule 5: Check compatibility notes
  ...
  componentsWithNotes.forEach((component) => {
    if (component && component.compatibilityNotes) {
      // Check if the note mentions a specific PSU requirement
      const psuMatch = component.compatibilityNotes.match(
        /(\d+)W\s*PSU/i
      );
  ...
  if (build.motherboard && build.ram) {
    ...
    if (moboSupport) {
      ...
      if (ramType) {
        ...
        } else if (ramType.includes("ddr4") && ramSupport.includes("ddr5") && !ramSupport.includes("ddr4")) {
          errors.push({
            type: "error",
            title: "RAM Type Mismatch",
...
// Quick check if a specific component would be compatible
export function checkComponentCompatibility(
  ...
  switch (category) {
    case "cpu":
      if (build.motherboard && component.socketType !== build.motherboard.socketType) {
        return {
          compatible: false,
