...
interface ComponentSelectorProps {
  /** Components grouped by category from the builder page */
...
export function ComponentSelector({
  ...
  // Extract categories from the componentsByCategory data
  const categories = useMemo(
    () => componentsByCategory.map((item) => item.category),
    [componentsByCategory]
  );

  // Create a map of category ID to components for quick lookup
  ...
  const handleAutoAdvance = () => {
    // Find next category in the order from page data
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="font-heading text-2xl font-bold flex items-center gap-3">
            ...
            <span>Select {categoryInfo?.name}</span>
          </h2>
