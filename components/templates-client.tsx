...
export function TemplatesClient({ templates }: TemplatesClientProps) {
  ...
  const handleApplyTemplate = (template: BuildTemplate) => {
    ...
    // Set experience level if not set
    if (!experienceLevel) {
      setExperienceLevel(template.targetLevel);
    }

    // Add all components from template using resolved components
  ...
  const getLevelInfo = (template: BuildTemplate) =>
    experienceLevelInfo[template.targetLevel];

  // Helper to get component by category from template
            ...
            >
              ...
              >
                ...
                <div className="aspect-video bg-gradient-to-br from-surface to-background flex items-center justify-center relative overflow-hidden">
                  <div className="absolute inset-0 bg-gradient-to-br from-neon-green/10 via-transparent to-neon-cyan/10 opacity-0 group-hover:opacity-100 transition-opacity" />
                  ...
                  <div className="absolute top-3 right-3">
                    <Badge className="bg-neon-green text-background font-bold text-sm">
                ...
                <CardContent className="p-4">
                  {/* Title */}
                  <h3 className="font-heading font-bold text-lg mb-1 group-hover:text-neon-green transition-colors">
                    {template.name}
                  </h3>
          ...
          <Card className="bg-surface/50 backdrop-blur-sm border-border/50">
            <CardContent className="p-6">
              <div className="flex items-start justify-between mb-6">
                ...
                <Badge className="bg-neon-green text-background font-bold text-lg px-4 py-2">
